...
object TopicCommand extends Logging {
  ...
  def createTopic(zkUtils: ZkUtils, opts: TopicCommandOptions) {
    ...
    val configs = parseTopicConfigsToBeAdded(opts)//将参数解析成Properties参数
  ...
  def parseReplicaAssignment(replicaAssignmentList: String): Map[Int, List[Int]] = {
    ...
    for (i <- 0 until partitionList.size) {
      ...
      if (duplicateBrokers.nonEmpty)//找到重复的
  ...
  class TopicCommandOptions(args: Array[String]) {
    ...
    val options = parser.parse(args : _*)//:_*作为一个整体，告诉编译器你希望将某个参数当作参数序列处理！例如val s = sum(1 to 5:_*)就是将1 to 5当作参数序列处理。
