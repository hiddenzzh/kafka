...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    private final long maxBlockTimeMs;//60,000
    ...
    private KafkaProducer(ProducerConfig config, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
        try {
            ...
            this.metadata = new Metadata(retryBackoffMs, config.getLong(ProducerConfig.METADATA_MAX_AGE_CONFIG),//300,000=5mins
                    true, true, clusterResourceListeners);
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG);//1048576
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);//33554432
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG));//none

            this.maxBlockTimeMs = config.getLong(ProducerConfig.MAX_BLOCK_MS_CONFIG);//60,000
            this.requestTimeoutMs = config.getInt(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG);//30,000
            this.transactionManager = configureTransactionState(config, logContext, log);
            int retries = configureRetries(config, transactionManager != null, log);
            int maxInflightRequests = configureInflightRequests(config, transactionManager != null);//5
            short acks = configureAcks(config, transactionManager != null, log);

            this.apiVersions = new ApiVersions();
            this.accumulator = new RecordAccumulator(logContext,
                    config.getInt(ProducerConfig.BATCH_SIZE_CONFIG),//16384
                    this.totalMemorySize,//33554432
                    this.compressionType,
                    config.getLong(ProducerConfig.LINGER_MS_CONFIG),//5
                    retryBackoffMs,
                    metrics,
                    time,
                    apiVersions,
            ...
            NetworkClient client = new NetworkClient(
                    new Selector(config.getLong(ProducerConfig.CONNECTIONS_MAX_IDLE_MS_CONFIG),//540,000
                            this.metrics, time, "producer", channelBuilder, logContext),
                    this.metadata,
                    clientId,
                    maxInflightRequests,//5
                    config.getLong(ProducerConfig.RECONNECT_BACKOFF_MS_CONFIG),//50
                    config.getLong(ProducerConfig.RECONNECT_BACKOFF_MAX_MS_CONFIG),//1000
                    config.getInt(ProducerConfig.SEND_BUFFER_CONFIG),//131072=128k
                    config.getInt(ProducerConfig.RECEIVE_BUFFER_CONFIG),//65536=64k
                    this.requestTimeoutMs,//30,000
                    time,
                    true,
                    apiVersions,
                    throttleTimeSensor,
            ...
            this.sender = new Sender(logContext,
                    ...
                    config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG),//1048576
                    acks,
                    retries,//0
                    metricsRegistry.senderMetrics,
                    Time.SYSTEM,
                    this.requestTimeoutMs,
                    config.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG),//100
    ...
    private static TransactionManager configureTransactionState(ProducerConfig config, LogContext logContext, Logger log) {
        ...
        if (idempotenceEnabled) {
            ...
            long retryBackoffMs = config.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG);
            transactionManager = new TransactionManager(logContext, transactionalId, transactionTimeoutMs, retryBackoffMs);//100
