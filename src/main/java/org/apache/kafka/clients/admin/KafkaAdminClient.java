...
import org.apache.kafka.clients.admin.extend.*;
...
public class KafkaAdminClient extends AdminClient {
    ...
    public DescribeConsumerGroupResult describeConsumerGroup(final String group,
        ...
        final KafkaFutureImpl<ConsumerGroupSummary> consumerGroupFuture = new KafkaFutureImpl<>();
        final long now = time.milliseconds();

        FindCoordinatorOptions opts = new FindCoordinatorOptions();
        opts.timeoutMs(options.timeoutMs());
        FindCoordinatorResult findCoordinatorResult = findCoordinator(group, opts);
        Node coordinator = null;
        try {
            coordinator = findCoordinatorResult.values().get();
        } catch (InterruptedException | ExecutionException e) {
            log.error(e.getMessage(), e);
        }

        SelfNodeProvider provider = new SelfNodeProvider(coordinator);
        runnable.call(new Call("describeConsumerGroup", calcDeadlineMs(now, options.timeoutMs()), provider) {
            @Override
            AbstractRequest.Builder createRequest(int timeoutMs) {
                return new DescribeGroupsRequest.Builder(Collections.singletonList(group));
            }

            @Override
            void handleResponse(AbstractResponse abstractResponse) {
                DescribeGroupsResponse response = (DescribeGroupsResponse) abstractResponse;
                DescribeGroupsResponse.GroupMetadata metadata = response.groups().get(group);
                if (metadata == null) {
                    consumerGroupFuture.completeExceptionally(new KafkaException("Response from broker contained no " +
                ...
                consumerGroupFuture.complete(consumerGroupSummary);
            }

            @Override
            void handleFailure(Throwable throwable) {
                consumerGroupFuture.completeExceptionally(throwable);
            }
        },now);

        return new DescribeConsumerGroupResult(consumerGroupFuture);
    }

    class SelfNodeProvider implements NodeProvider {
        private Node node;

        public SelfNodeProvider(Node node) {
            this.node = node;
        }
    ...
    public FindCoordinatorResult findCoordinator(final String group,
        ...
        final KafkaFutureImpl<Node> nodeFuture = new KafkaFutureImpl<>();
        final long now = time.milliseconds();
        runnable.call(new Call("findCoordinator", calcDeadlineMs(now, options.timeoutMs()),
            ...
            void handleResponse(AbstractResponse abstractResponse) {
                ...
                if (errors == Errors.NONE) {
                    ...
                    nodeFuture.complete(coordinator);
                } else if (errors == Errors.COORDINATOR_NOT_AVAILABLE) {
                    nodeFuture.completeExceptionally(new TimeoutException("The consumer group command timed out while" +
                            " waiting for group to initialize:", errors.exception()));
                } else {
                    nodeFuture.completeExceptionally(errors.exception());
                }
            }
        ...
        return new FindCoordinatorResult(nodeFuture);
    }

    public ListGroupOffsetsResult listGroupOffsets(final String group,
                                                  final ListGroupOffsetsOptions options) {
        final KafkaFutureImpl<Map<TopicPartition, Long>> future = new KafkaFutureImpl<>();
        final long now = time.milliseconds();
        ...
        SelfNodeProvider provider = new SelfNodeProvider(coordinator);
        runnable.call(new Call("listGroupOffsets",calcDeadlineMs(now,options.timeoutMs()),provider) {
            @Override
            AbstractRequest.Builder createRequest(int timeoutMs) {
                return OffsetFetchRequest.Builder.allTopicPartitions(group);
            }
            ...
            void handleResponse(AbstractResponse abstractResponse) {
                ...
                if (response.hasError()) {
                    ...
                    response.responseData().forEach((tp, partitionData)-> map.put(tp, partitionData.offset));
                    future.complete(map);
                }
            }
        ...
        return new ListGroupOffsetsResult(future);
    }
